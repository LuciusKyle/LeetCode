...
#endif
    return wordBreak(s, words_set);
  }

 private:
  bool wordBreak(const string &s,
                 const unordered_set<string> &words_set) const {
    if (0 < words_set.count(s))
      return true;
    else
      for (auto iter = s.cbegin() + 1; iter != s.cend(); ++iter)
        if (wordBreak({s.cbegin(), iter}, words_set) && wordBreak({iter, s.cend()}, words_set))
          return true;
    return false;
  }
};
...
int main(void) {
  ...
  Solution sln;
  sln.wordBreak("fohhemkkaecojceoaejkkoedkofhmohkcjmkggcmnami", {
