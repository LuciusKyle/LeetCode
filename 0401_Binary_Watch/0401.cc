...
int main(void) {
  ...
  for (int h = 0; h < 12; ++h) {
    for (int m = 0; m < 60; ++m) {
      if (h == m && m == 0) continue;
      sprintf(temp_str, "%d:%02d", h, m);
      output[count_bit_1(h) + count_bit_1(m)].push_back(temp_str);
    }
  }
