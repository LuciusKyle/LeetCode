...
public:
	...
	int ladderLength(const string beginWord, const string endWord, const vector<string>& wordList) {
		...
		return 0;
		////////////////////////////////////////////////////////////////////////////////////////////////////
		auto rtn = findLadders(beginWord, endWord, wordList);
		return rtn.empty() ? 0 : static_cast<int>(rtn.back().size());
	}
private:
	void findLadders(const string &beginWord, const string &endWord, const vector<string> &wordList, vector<int> &used_words, vector<string> &buffer, vector<vector<string>> &rtn) {
		buffer.push_back(beginWord);
		if (!rtn.empty() && rtn.back().size() < buffer.size()) {
			return;
		}
		if (beginWord == endWord) {
			if (rtn.empty() || buffer.size() == rtn.back().size()) {
				rtn.push_back(buffer);
			}
			else if (buffer.size() < rtn.back().size()) {
				rtn.clear();
				rtn.push_back(buffer);
			}
			return;
			...
			for (size_t i = 0; i < wordList.size(); ++i) {
				if (used_words[i] == 0) {
					if (ValidTransformation(beginWord, wordList[i])) {
						used_words[i] = 1;
						findLadders(wordList[i], endWord, wordList, used_words, buffer, rtn);
						used_words[i] = 0;
					}
				}
			}
